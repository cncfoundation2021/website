...
class MenuManager {
    ...
    async init() {
        try {
            ...
            if (currentPath.includes('/offerings/')) {
                ...
                // Info pages are one level deep, so go up one level
                sitemapPath = '../config/sitemap.json';
            }
            
            // Add cache-busting parameter
            const cacheBuster = `?v=${Date.now()}`;
            const response = await fetch(sitemapPath + cacheBuster);
            this.sitemap = await response.json();
            console.log('Sitemap loaded successfully from:', sitemapPath);
            console.log('Sitemap leftPane items:', this.sitemap.sitemap.leftPane);
            console.log('Sitemap topNav items:', this.sitemap.sitemap.topNav);
        } catch (error) {
    ...
    getLeftPane() {
        ...
        const items = this.sitemap.sitemap.leftPane.map(item => ({
            title: item.title,
            slug: item.slug,
            route: `/info/${item.slug}.html`
        }));
        console.log('getLeftPane() returning:', items);
        return items;
    }
    ...
    getTopTabs() {
        ...
        const items = this.sitemap.sitemap.topNav
            .filter(item => {
        ...
        console.log('getTopTabs() returning:', items);
        return items;
    }

    // Helper method to determine correct route for a slug
    getRouteForSlug(slug) {
        // HOME always goes to main homepage (Welcome hero, feature & suggested products)
        if (slug === 'home') {
            return '/';
        }
        ...
        const slugsWithIndex = [
            ...
            'e-bussiness', 'cnc-bazar', 'product-catalog'
        ];
